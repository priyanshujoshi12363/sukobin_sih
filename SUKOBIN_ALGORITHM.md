...
- **Multilingual**: complete, and worth stating precisely because "multilingual"
  was previously doing a lot of work in this document.

  | | |
  |---|---|
  | App UI | 635 strings x 9 languages = 5,715, across all four apps. 100% coverage, no gaps. |
  | Alerts | 7 templates x 10 languages, picked by the recipient's `preferredLanguage` |
  | Driver road warnings | 4 templates x 10 languages, with a separate "speak" variant |
  | Spoken reports | any of the ten in, English out, original words kept |
  | Switching | one shared picker, per-app locale, survives restart |

  The UI translations are machine-generated by `android/tools/translateStrings.js`
  using the same model the backend uses, and every file says so in its header.
  They need a native speaker before this reaches real officers and drivers.

  What the pipeline does guarantee is that they are *safe*:
  `android/tools/checkTranslations.js` verifies every one of 5,841 units and
  fails the build on a format specifier that changed, a plural missing a
  quantity, or an unescaped apostrophe. A dropped `%1$s` throws
  IllegalFormatException at runtime, on one screen, for one language only — it
  would never show up in a build, so it is checked before one. Anything the
  model gets wrong is dropped rather than shipped, and Android falls back to
  English for that string.

  Two honest notes. Meiteilon is written in Bengali script here rather than
  Meetei Mayek. And the platform calls Kokborok `kok` throughout, which is
  really Konkani's ISO code (Kokborok is `trp`); the codebase's own name was
  kept rather than diverging from the API mid-project.
